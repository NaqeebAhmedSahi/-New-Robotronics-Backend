...
const verifyTokenAdmin = async(req, res, next) => {
    ...
    if(req.headers.authorization.startsWith("Bearer ")){
      const token = req.headers.authorization.split(' ')[1] // get the second element which is the token itself(without the "Bearer" text)
        ...
        if(err) return res.status(403).json({msg: `Wrong or expired token.${err}`})
          ...
          console.log(req.user)
            if(!data.isAdmin){ // only admin can do certain things
                return res.status(403).json({msg: `You are not admin.${err}`})
            } 
            req.user = data // an object with only the user id as its property
            next()
        }
